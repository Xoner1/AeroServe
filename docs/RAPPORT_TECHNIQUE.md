...
| `CHEF_CUISINE` | Chef de Cuisine | Angular Web (Cuisine) | Gestion des produits culinaires (FOOD), écriture des recettes, planification du menu, validation de production. |
| `CHEF_MAGASIN` | Chef Magasin | Angular Web + Mobile | Contrôle des stocks du magasin central, mouvements d'entrée/sortie, traitement FIFO, suivi des DLC. |
| `RESPONSABLE_ACHAT` | Responsable Achat | Angular Web (Achats) | Approbation commerciale des produits, prix de vente, prévisions d'achat IA. |
...
graph LR
  ...
  CHEF_CUISINE[Chef de Cuisine] -->|Concoit| recipes[Recettes & Plats FOOD]
...
- `assigned_to` (Foreign Key, Nullable) : Destinataire (Cuisine ou Magasin).
...
- **Fonctionnalités :** Saisie des entrées de marchandises en réserve avec dates de péremption obligatoires. Traitement des bons de commande internes émis par la cuisine.

### 5.5 Responsable Achat

- **Composants Angular :** `category.ts`, `products.component.ts`, `products-validation`.
- **Fonctionnalités :** Approbation finale des nouveaux produits saisis en réserve, fixation des prix de vente, et consultation des analyses de stocks de l'assistant IA.
...
```mermaid
flowchart LR
  Draft[Creation Commande] -->|Statut EN_ATTENTE| Assigned[Assignation auto : Cuisine ou Magasin]
...
| `PUT` | `/api/stocks/{stock}/threshold` | Magasin, Cuisine, F&B | `min_threshold` | `{message}` | Modification du seuil d'alerte. |
...
- **Restriction de visibilité :** L'API filtre l'indexation des commandes selon le rôle. Le `SUPER_ADMIN` a accès complet, le `RESPONSABLE_FB` accède à ses créations et aux commandes de ses points de vente, le `CHEF_CUISINE` / `CHEF_MAGASIN` accèdent aux bons créés ou assignés à leur poste, et les autres rôles (comme `CAISSIER`) ne visualisent que leurs propres commandes.
